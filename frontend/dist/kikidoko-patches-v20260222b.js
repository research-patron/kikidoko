(() => {
  ...
  async function fetchSnapshotWithFailover(requestUrl, init) {
    ...
    try {
      const liteResponse = await originalFetch(`${PATH_SNAPSHOT_LITE}${search}`, init);
      if (!liteResponse.ok) {
      ...
      try {
        const fullResponse = await originalFetch(requestUrl, init);
        if (!fullResponse.ok) {
        ...
        updateSnapshotState({
          ...
          lastError: String(fullError?.message || fullError || "snapshot_full_failed"),
        });
        throw fullError;
      }
    }
  ...
  function installFetchInterceptor() {
    ...
    window.fetch = (input, init) => {
      ...
      if (pathname === PATH_SNAPSHOT_FULL_GZIP || pathname === PATH_SNAPSHOT_FULL_JSON) {
  ...
  function installSnapshotClickGuard() {
    ...
    const shouldBlock = (target) => {
      ...
      if (!target.closest(".jp-map-shape") && !target.closest(".jp-map-marker") && !target.closest(".prefecture-row")) {
        return false;
      }
      return getSnapshotState().status !== "ready";
    };
